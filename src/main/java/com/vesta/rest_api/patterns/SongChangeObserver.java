...
/**
 * SongChangeObserver is a class that extends Vestaboard and implements the Observer interface.
 * It is responsible for updating the Vestaboard display when a song change event occurs.
 * 
 * The class uses a Logger to log information and has a constructor that takes a vestaboardKey as a parameter.
 * 
 * The updateBoard method updates the Vestaboard display with the currently playing song and the next song.
 * 
 * The update method is overridden from the Observer interface and handles different types of events:
...
public class SongChangeObserver extends Vestaboard implements Observer {
	...
	/**
	 * Updates the Vestaboard display with the currently playing song and the next song.
	 *
