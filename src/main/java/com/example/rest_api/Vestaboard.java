...
public class Vestaboard {
	...
	public String getState() {
		...
		try (CloseableHttpClient client = HttpClients.createDefault()) {
			...
			Gson gson = new Gson();
			Map<String, Map<String, String>> map = gson.fromJson(result, HashMap.class);
	...
	public String sendMessage(String message) {
		...
		try (CloseableHttpClient client = HttpClients.createDefault()) {
			...
			final String messageBody = String.format("{\"text\": \"%s\"}", message);
			...
			try {
				...
				e.printStackTrace();
				System.err.println("This might be because you are trying to submit text that is already on the board.");
				return null;
			}
